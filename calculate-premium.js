...
async function getScore(land, options) {
  ...
  const weatherDataResponse = await Functions.makeHttpRequest({
    url: `http://api.weatherapi.com/v1/forecast.json?key=${secrets.WEATHER_API_KEY}&q=${location}&days=${
...
const premium = coverage / (score * score)

return Functions.encodeUint256(Math.round(premium))
